...
public class CPU {
	...
	public void update(){
		...
		//se estiver ocupado
			...
			//se alguem estiver esperando,realizar escalonamento
			if(!tarefas.isEmpty()){
				Tarefa e = tarefas.get(0);
				this.escalonarEmissor(e);			
	...
	private void escalonarEmissor(Tarefa e){
		Random rn = new Random();
		boolean achou = false;
		int i = 0;
		while (!achou && i < N){
			...
			if (index!=id){
				...
				CPU p = processadores.get(index);
				p.incSondRecebida();
				//se nao tiver ocupado, mandar tarefa para p
				if (!p.isBusy()){
					achou = true;
					p.addTarefa(e);
				}
